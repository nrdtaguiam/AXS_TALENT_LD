...
async def run_matchmaking(req: Optional[MatchmakingRequest] = None, talent_ids: Optional[str] = None):
    ...
    async def trace_generator():
        ...
        # 1. Ontology Agent Hand-off
        yield "data: ONTOLOGY_AGENT | MAPPED: Translating academic outcomes and rubrics into corporate frameworks.\n\n"
        await asyncio.sleep(0.4)
        yield "data: ONTOLOGY_AGENT | MAPPED: Translating curriculum data into competency framework indices.\n\n"
        await asyncio.sleep(0.4)
        
        # 2. Diagnostic Agent Hand-off
        for candidate in candidates_list:
            if candidate["status"] != "DEPLOYMENT_READY" or candidate["id"] == "T011":
                continue
            name = candidate["name"]
            gaps_str = ", ".join(candidate["gaps"])
            yield f"data: DIAGNOSTIC_AGENT | PROFILED: Computed {name}'s current proficiency matrix vs Target Role. Gap identified: {gaps_str}.\n\n"
            await asyncio.sleep(0.4)
            
        # 3. Routing Agent Hand-off
        for candidate in candidates_list:
            if candidate["status"] != "DEPLOYMENT_READY" or candidate["id"] == "T011":
                continue
            name = candidate["name"]
            ...
            if role == "Risk Analyst":
                bypassed = "SQL/Quantitative Modeling"
                sprint = "2-week Advanced Credit Risk Simulation Sprint"
            elif role == "Project Manager":
                bypassed = "Stakeholder Management/Jira"
                sprint = "3-week Enterprise Real Estate Project Delivery Sprint"
            else:
                bypassed = "Python/Data Pipelines"
                sprint = "4-week Telecom Data Streams (Globe AWS Stack) Sprint"
            yield f"data: ROUTING_AGENT | ROUTED: Bypassed {bypassed} modules. Generated a hyper-focused {sprint} learning sprint.\n\n"
            await asyncio.sleep(0.4)
            
        # 4. Predictive Matchmaker Evaluation
        matches = matchmaker.execute_matchmaking(passports)
        min_confidence = 0.0
        if req and req.filters:
            min_confidence = req.filters.get("min_readiness", 0.0)
        filtered_matches = [m for m in matches if m["match_confidence"] >= min_confidence]
        ...
        for candidate in candidates_list:
            ...
            yield f"data: PREDICTIVE_MATCHMAKER | MATCHED: Calculated {readiness}% alignment index to {role} vacancy at {bu}.\n\n"
            await asyncio.sleep(0.4)
            
        yield "data: SYSTEM | COMPLETE: Matchmaking simulation pipeline finished successfully.\n\n"
