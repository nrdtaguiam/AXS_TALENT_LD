...
* **Universal Skills Taxonomy (`agents/ontology_agent.py`)**: Live agent that dynamically maps academic courses to corporate standard competencies using the Gemini `gemini-2.5-flash` model, backed by clean fallbacks.
* **Skill Passport Generator (`agents/diagnostic_agent.py`)**: Dynamic profiling agent that issues generative LLM calls validated against a strict Pydantic schema structure (`readiness_score`, `gaps`, `categories`, and `skill_matrix`) to output a JSON "Skill Passport".
* **Personalized Learning Routes (`agents/routing_agent.py`)**: Dynamically queries Gemini to construct structured 4-week learning pathways based on candidate current skills and gaps.
* **Predictive Corporate Matchmaker (`agents/predictive_matchmaker.py`)**: Real LLM matchmaking engine powered by the `google-genai` SDK and `gemini-2.5-flash` at a temperature of `0.2` to enforce analytical data consistency. Enforces strict structured responses matching our frontend schemas using Pydantic models.
...
### 2. Live LLM Orchestration & Fallback
The `OntologyAgent`, `DiagnosticAgent`, and `RoutingAgent` are configured with a hardcoded verified API key (`"XX.XXXXXXXXXXXXX"`) to initialize the live `google-genai` client. 
*Note: In the absence of a live connection or valid API credentials, the agents automatically intercept errors/timeouts and transition gracefully to structured heuristic fallback generators. This ensures that the platform functions seamlessly and remains offline-resilient.*
